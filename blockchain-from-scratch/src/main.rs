...
fn main() {
    ...
    let mempool: HashMap<H256, SignedTransaction> = HashMap::new();
    ...
    let mut worker_ctx =
        network::worker::Worker::new(p2p_workers, msg_rx, &server, &blockchain, &mempool);
    worker_ctx.start();
    ...
    let (generator_ctx, generator, finished_block_chan) =
        txgenerator::new(&blockchain, &mempool, &server, p2p_addr.clone());
    println!("before starting generator");
    generator_ctx.start();
    println!("after starting generator");
    ...
    ApiServer::start(api_addr, &miner, &server, &blockchain, &mempool, &generator);
